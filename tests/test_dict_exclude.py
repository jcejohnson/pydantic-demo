...
class TestDictExclude:
    ...
    def test_exclude_cast(self):
        # Success
        library = Library(movies=movies)
        assert library.dict(exclude_unset=True, exclude={'movies': {0: {'cast'}}}) \
            == dict({'movies': movies_title_only})
        library = Library(movies=movies)
        assert library.dict(exclude_unset=True, exclude={'movies': {0: {'cast': ...}}}) \
            == dict({'movies': movies_title_only})
        assert library.dict(exclude_unset=True, exclude={'movies': {'__all__': {'cast'}}}) \
            == dict({'movies': movies_title_only})
        library = Library(movies=movies)
        assert library.dict(exclude_unset=True, exclude={'movies': {-1: {'cast'}}}) \
            == dict({'movies': movies_title_only})
